...
authRouter.post("/verify", (req, res) => {
  ...
  if (!challenge) {
    return res
      .status(401)
      .json({ error: "no challenge issued for this address" });
  }
...
async function verifyPresentation(p: Presentation) {
  ...
  let holder: string;
  try {
    holder = ethers.verifyMessage(holderMsg, p.holderSignature).toLowerCase();
  } catch {
    return { ok: false, reason: "malformed holder signature" };
  }
  ...
  let issuer: string;
  try {
    issuer = ethers.verifyMessage(credMsg, p.credentialSignature).toLowerCase();
  } catch {
    return { ok: false, reason: "malformed credential signature" };
  }
