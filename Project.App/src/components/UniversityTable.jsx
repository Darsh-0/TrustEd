      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {universities.map((uni) => (
            <tr key={uni.address} className="border-b border-zinc-800/60 last:border-0 hover:bg-zinc-800/30">
              <td className="px-6 py-3 font-mono text-xs text-zinc-300" title={uni.address}>
                {truncateAddr(uni.address)}
              </td>
              <td className="px-4 py-3 text-zinc-200">{uni.name}</td>
              <td className="px-4 py-3 text-zinc-400">{uni.country || '—'}</td>
              <td className="px-4 py-3 font-mono text-xs text-zinc-400" title={uni.publicKey}>
                {uni.keyType || '—'}
              </td>
