	...
	return (
		<section className="mx-auto max-w-container px-6 py-12 sm:px-10">
			<div className="flex flex-wrap items-end justify-between gap-4">
				...
				<span className="rounded-full border border-primary/20 bg-primary-container/10 px-3 py-1.5 font-label text-xs font-semibold text-primary">
					{count} accredited
				</span>
			</div>
			...
			{wrongNetwork && (
				<div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800">
					...
					<button className="btn-primary py-2" onClick={() => wallet.switchNetwork(31337)}>
						Switch Network
					</button>
				</div>
			)}
