...
export default function SharePage() {
  ...
  async function share(id) {
    setShareState((prev) => ({
      ...prev,
      [id]: { loading: true, link: null, error: null },
    }));
    try {
      const link = await createShareLink(id);
      setShareState((prev) => ({
        ...prev,
        [id]: { loading: false, link, error: null },
      }));
    } catch (e) {
      const error = e.code === 4001 ? "Signature rejected." : e.message;
      setShareState((prev) => ({
        ...prev,
        [id]: { loading: false, link: null, error },
      }));
    }
  }
        ...
        creds.map((c) => {
          ...
          return (
            <div key={id} className="card mt-4">
              ...
              {state.link && (
                <div className="mt-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button
                      onClick={() => copy(id, state.link)}
                      className="btn-outline whitespace-nowrap cursor-pointer"
                    >
              ...
              {state.error && (
                <p className="mt-3 rounded-lg border border-error-container bg-error-container/40 px-3 py-2 text-sm text-on-error-container">
                  {state.error}
                </p>
              )}
            </div>
